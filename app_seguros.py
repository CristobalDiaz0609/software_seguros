                ...
                with st.form(key=f"form_edit_{id_poliza}_{idx}"):
                    ...
                    if btn_guardar_cambios:
                        try:
                            conn = get_connection()
                            cursor = conn.cursor()
                            cursor.execute("UPDATE clientes SET nombre_completo = %s, rut = %s, telefono = %s, email = %s WHERE id_cliente = %s", (edit_nombre, edit_rut, edit_tel, edit_email, id_cliente))
                            ...
                            cursor.execute("UPDATE polizas SET numero_poliza = %s, id_compañia = %s, ramo = %s, materia_asegurada = %s, monto_prima_anual = %s, monto_comision = %s, fecha_vencimiento = %s WHERE id_poliza = %s", (edit_poliza, id_comp_actualizada, edit_ramo, edit_materia, edit_prima, edit_comision, edit_venc, id_poliza))
                            conn.commit()
                            cursor.close()
                            conn.close()
